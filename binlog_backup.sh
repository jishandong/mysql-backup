...
backup_dir="/data/mysql-master/data/"
...
rsync_mysql_binlog()
{
    ...
    do                
        ...
        ### 同步 ###
        rsync -avz --progress --delete --include="*-bin.*" --exclude="*" $serverPath -e "ssh -p "${ssh_port}" -i "${id_rsa} ${id_rsa_user}@${j}:$clientPath >>${logfile} 2>&1 
